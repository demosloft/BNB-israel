...
//get property status=0
$ru_list = $wpdb->get_results("SELECT * FROM `ru_all_list` WHERE `status`='0' limit 2");

if($ru_list)
foreach($ru_list as $ru_lst)
{
	$pid = $ru_lst->ru_id;
	...
	//cleaning Fee
	update_post_meta( $listing_post_id, 'cleaning_fee', $cleaningPrice );
	...
	$sql = "UPDATE `ru_all_list` SET `status` = '1' WHERE `id`='".$ru_lst->id."'";
	$wpdb->query($sql);
