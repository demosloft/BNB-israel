...
*/
	    ...
	    */
        ...
        /*
         * replaced by tagRule TODO check
        SHORTPIXEL_AI_DEBUG && $this->logger->log("******** REGEX PARSER replace_wc_gallery_thumbs ********* Content len: " . strlen($content));

        $content = preg_replace_callback(
            '/\<(div|figure|li)[^\<\>]*?\sdata-thumb(?:nail|)\=(?:\"|\')(.+?)(?:\"|\')(?:.+?)\>/s',
            array($this, 'replace_wc_gallery_thumbs'),
            $content
        );

        //$integrations = ActiveIntegrations::_();
        //$theme = $integrations->get('theme');



        SHORTPIXEL_AI_DEBUG && $this->logger->log("******** REGEX PARSER replace_background_image_from_tag ********* Content len: " . strlen($content));
        ...
        $bkImgRegex = '/\<(' . $bgTagsList . ')([^\<\>]*?)\b(background-image|background)(\s*:(?:[^;]*?[,\s]|\s*))url\((?:\'|")?([^\'"\)]*)(\'|")?\s*\)(.*?)\>/s';
        $contentReplaced = preg_replace_callback( $bkImgRegex, array($this->cssParser, 'replace_background_image_from_tag'), $content);
        if(strlen($contentReplaced)) { // better safe than sorry
            $content = $contentReplaced;
        } else {
            //this can happen if there are HUGE tags like for example a <form with data-product_variations having a large number of variations (HS# 59186)
            SHORTPIXEL_AI_DEBUG && $this->logger->log("******** REGEX PARSER replace_background_image_from_tag FAILED using regex: " . $bkImgRegex);
        }

        if($theme == 'CROWD 2' || $theme == 'Lovely 2') {
            $regex = str_replace(array('{{TAG}}', '{{ATTR}}'), array('div|a|span', 'style'), $this->regexes['srcset']);
            $this->logger->log("CROWD 2 theme - regex $regex Content len: " . strlen($content));
        ...
        if($integrations->get('wp-bakery')) {
            SHORTPIXEL_AI_DEBUG && $this->logger->log("WP BAKERY data-ultimate-bg Content len: " . strlen($content));
            $content = preg_replace_callback(
        ...
        if($this->ctrl->settings->behaviour->replace_method !== 'src') { //srcset has to be checked too, in some cases the srcset wp hook isn't called...
            ...
            SHORTPIXEL_AI_DEBUG && $this->logger->log("SRCSET REGEX: $regex Content len: " . strlen($content));
            $content = preg_replace_callback($regex,
        ...
        */
        ...
        /*
         * replaced by tagRule TODO check
        if ($integrations->get('envira')) {
            $regex = str_replace(array('{{TAG}}', '{{ATTR}}'), array('img', 'data-envira-srcset'), $this->regexes['srcset']);
            SHORTPIXEL_AI_DEBUG && $this->logger->log("ENVIRA REGEX: $regex Content len: " . strlen($content));
            $content = preg_replace_callback($regex,
                array($this, 'replace_custom_srcset'),
                $content
            );
        ...
        if($theme == 'Jupiter') {
            ...
            $regex = str_replace(array('{{TAG}}', '{{ATTR}}'), array('img', 'data-mk-image-src-set'), $this->regexes['srcset']);
            SHORTPIXEL_AI_DEBUG && $this->logger->log("Jupiter data-mk: $regex Content len: " . strlen($content));
            $content = preg_replace_callback($regex,
                array($this, 'replace_custom_json_attr'),
                $content
            );
            $regex = str_replace(array('{{TAG}}', '{{ATTR}}'), array('div', 'data-mk-img-set'), $this->regexes['srcset']);
            $content = preg_replace_callback($regex,
                array($this, 'replace_custom_json_attr'),
                $content
            );
        } elseif(strpos($theme, 'Divi') === 0) {
            TODO decide how to handle the affected tags with tag rules - don't forget the lazy AJAX tags that have to be marked in the settings...
            $this->ctrl->affectedTags->add('div', 2);
            //Divi - Elegant Themes' custom attribute
            $regex = str_replace(array('{{TAG}}', '{{ATTR}}'), array('img', 'data-et-multi-view'), $this->regexes['srcset']);
            SHORTPIXEL_AI_DEBUG && $this->logger->log("DIVI data-et-multi-view regex: $regex Content len: " . strlen($content));
            $content = preg_replace_callback($regex,
                array($this, 'replace_custom_json_attr'),
                $content
            );
        ...
        if($integrations->get('acf')) {
            //ACF moves src's from hidden images to FIGURE backgrounds
            $this->ctrl->affectedTags->add('figure', 2);
        }
        */

        /*
         * replaced by tagRule TODO check
        if($integrations->get('elementor')) {
            //Elementor can pass image URLs in sections' or divs' data-settings
            $regex = str_replace(array('{{TAG}}', '{{ATTR}}'), array('header|section|div', 'data-settings|data-options'), $this->regexes['srcset']);
            SHORTPIXEL_AI_DEBUG && $this->logger->log("ELEMENTOR data-settings regex: " . $regex . " Content len: " . strlen($content));
            $content = preg_replace_callback($regex,
                array($this, 'replace_custom_json_attr'),
                $content
            );
        ...
        if($integrations->get('real3d-flipbook')) {
            //Elementor can pass image URLs in sections' or divs' data-settings
            $regex = str_replace(array('{{TAG}}', '{{ATTR}}'), array('div', 'data-flipbook-options'), $this->regexes['srcset']);
            SHORTPIXEL_AI_DEBUG && $this->logger->log("REAL 3D FLIPBOOK regex: " . $regex . " Content len: " . strlen($content));
            $content = preg_replace_callback($regex,
                array($this, 'replace_custom_json_attr'),
                $content
            );
        ...
        if($integrations->get('woocommerce') && preg_match('/\<(' . $variationTags . ')\b[^>]+\bdata-product_variations=/is', $content)) {
            //there are product variations
            $regex = str_replace(array('{{TAG}}', '{{ATTR}}'), array($variationTags, 'data-product_variations'), $this->regexes['srcset']);
            SHORTPIXEL_AI_DEBUG && $this->logger->log("WOO REPLACE VARIATIONS regex: " . $regex . " Content len: " . strlen($content));
            $content = preg_replace_callback($regex,
        ...
        */
        ...
        /*if($this->integrations->get('modula')) {
            ...
            if ( $this->ctrl->settings->areas->js2cdn ) {
                ...
                if(preg_match($regex, $script,$matches)) {
                    ...
                    if(   strpos($url, parse_url($this->ctrl->settings->behaviour->api_url, PHP_URL_HOST)) === false
                        ...
                        $cdnUrl = $this->ctrl->get_api_url(false, false, $this->ctrl->get_extension( $url )) . '+v_' . $this->ctrl->cssCacheVer . '/' . ShortPixelUrlTools::absoluteUrl($url);
	...
	*/
    ...
    protected function _replace_images($tag, $attr, $text, $url, $q = '', &$noresize = false) {
	    ...
	    if ( strpos( $text, 'attachment-glow-featured-slider-img' ) ) {
		    $this->ctrl->affectedTags->add( 'figure', \ShortPixel\AI\AffectedTags::SRC_ATTR); //the Glow Pro moves the image in <figure> tags
            $this->tagRule->used['figure'] = \ShortPixel\AI\AffectedTags::SRC_ATTR;
	    }
        ...
        if((isset($sizes['thumb']) || $eager) && $this->tagRule->mergeAttr) {
            ...
            if(preg_match('/\b' . $this->tagRule->mergeAttr . '=(\"|\')(.*?)(?:\"|\')/s', $text, $matches) && isset($matches[2]) && strlen(trim($matches[2]))){
                ...
                foreach($items as $item) {
                    ...
                    if($eager) {
                        $apiUrl = $this->ctrl->get_api_url(false, false, false, $this->tagRule->getCustomCompression());
                        $atirReplacement .= (strlen($atirReplacement) ? ', ': ' ' . $this->tagRule->mergeAttr . '="')
                            . ($this->ctrl->urlIsApi($parts[0]) ? $parts[0] : $apiUrl . '/' . ShortPixelUrlTools::absoluteUrl($url))
        ...
        if($this->tagRule->mergeAttr) {
            $str = preg_replace('/' . $this->tagRule->mergeAttr . '=(\"|\').*?(\"|\')/s', $atirReplacement, $str);
        ...
        $this->ctrl->affectedTags->add($tag, \ShortPixel\AI\AffectedTags::SRC_ATTR);
        $this->tagRule->used[$tag] = \ShortPixel\AI\AffectedTags::SRC_ATTR;
