		...
		<div class="shortpixel-settings-wrap">
				...
				<div class="box_content">
					<?php
						...
						if ( $domain_status->HasAccount && $domain_usage ) {
                            ...
                            $total_monthly = $is_our_cdn ? ShortPixelDomainTools::credits2bytes($domain_usage->quota->monthly->total) : number_format($domain_usage->quota->monthly->available);
                            $total_monthly_available = $is_our_cdn ? ShortPixelDomainTools::credits2bytes($domain_usage->quota->monthly->available) : number_format($domain_usage->quota->monthly->available);
                            $total_monthly_used = $is_our_cdn ? ShortPixelDomainTools::credits2bytes($domain_usage->quota->monthly->used) : number_format($domain_usage->quota->monthly->used);
			...
			<div class="shortpixel-settings-tabs">
				<h2 class="nav-tab-wrapper" id="wpspai-tabs">
					...
					<button id="clear_css_cache" class="spai_top_actions" data-pressed-text="<?= __( 'Clearing...', 'shortpixel-adaptive-images' ); ?>"><?=
                        __( 'Clear CSS Cache', 'shortpixel-adaptive-images' );
                    ?></button>
						...
						<table class="form-table">
							...
							<tr>
								...
								<td>
                                    ...
                                    />
									<p class="description">
										<?= __( 'Do <strong>not</strong> change this unless you plan on using your own CDN and you have already configured it to use the ShortPixel.ai service. Check out <a href="https://help.shortpixel.com/article/180-can-i-use-a-different-cdn-with-shortpixel-adaptive-images" target="_blank">here</a> or <a href="https://help.shortpixel.com/article/200-setup-your-stackpath-account-so-that-it-can-work-with-shortpixel-adaptive-images-api" target="_blank">here</a> for examples.',
											'shortpixel-adaptive-images' ); ?>
									</p>
                                    <div class="notification_popup hidden">
                                        <p class="text">
                                            <?= __( 'If you will use your own CDN, ShortPixel will switch from counting the traffic (1 credit = 5MB) to counting the number of scaled & optimized images. Depending on the site, this can increase considerably the number of credits used. If you\'re instead using a DNS CNAME to our CDN, the traffic will continued to be counted as usual. <a href="https://help.shortpixel.com/article/460-spai-new-how-are-the-credits-counted" target="_blank">How are credits counted?</a>',
                                            'shortpixel-adaptive-images' ); ?>
                                        </p>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="spai-inline-help">
                                        <span class="dashicons dashicons-editor-help" title="Inline help"
                                              data-link="https://help.shortpixel.com/article/507-spai-settings-size-breakpoints"></span></div>
                                    <input
                                            id="size_breakpoints"
                                            type="checkbox"
